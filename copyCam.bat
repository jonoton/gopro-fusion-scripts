...
powershell.exe -Command "%~dp0utils\DeviceFiles.ps1" -copyAll -deviceName "%FRONT_NAME%" -sourceFolder "%FRONT_DIR%" -targetFolder "%DST_FOLDER_FRONT%" -filter "%FILE_FILTER%"

powershell.exe -Command "%~dp0utils\DeviceFiles.ps1" -copyAll -deviceName "%BACK_NAME%" -sourceFolder "%BACK_DIR%" -targetFolder "%DST_FOLDER_BACK%" -filter "%FILE_FILTER%"
